...
import AlphaHumanThreadHistoryHandler from '../alphaHumanThread/AlphaHumanThreadHistoryHandler';
...
import SummarizeHistoryHandler from '../middle/summarize/SummarizeHistoryHandler';
...
const Main = ({
  ...
  useEffect(() => {
    if (isMasterTab && isSynced) {
      ...
      getActions().initAlphaHumanSocket();
    }
  ...
  const handleHashChange = useLastCallback(() => {
    const parsedLocationHash = parseLocationHash(currentUserId);

    if (!parsedLocationHash) {
      // Hash cleared - close summarize if open
      const tabState = getGlobal().byTabId[getCurrentTabId()];
      if (tabState?.summarize) {
        // History handler will manage closing
      }
      return;
    }

    if ('isSummarize' in parsedLocationHash && parsedLocationHash.isSummarize) {
      openSummarizePage();
      return;
    }

    if ('isAlphaHumanThread' in parsedLocationHash && parsedLocationHash.isAlphaHumanThread) {
      const { threadId } = parsedLocationHash;
      if (threadId) {
    ...
    if ('chatId' in parsedLocationHash) {
      const { chatId, threadId, type } = parsedLocationHash;
      if (chatId) {
        openThread({
          chatId,
          threadId: threadId || 0,
  ...
  useEffect(() => {
    handleHashChange();
  }, [currentUserId, handleHashChange]);
