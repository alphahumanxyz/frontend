...
import AlphaHumanSmartChatScreen from './alphaHumanSmartChat/AlphaHumanSmartChatScreen';
...
const RightColumn: FC<OwnProps & StateProps> = ({
  ...
  const isAgentChat = contentKey === RightColumnContent.AgentChat;
  const isThreadHistory = contentKey === RightColumnContent.ThreadHistory;
  const isOverlaying = windowWidth <= MIN_SCREEN_WIDTH_FOR_STATIC_RIGHT_COLUMN;

  const [shouldSkipTransition, setShouldSkipTransition] = useState(!isOpen);
  const [isThreadHistoryOverlayOpen, setIsThreadHistoryOverlayOpen] = useState(false);
  ...
  const openThreadHistoryOverlay = useLastCallback(() => {
    setIsThreadHistoryOverlayOpen(true);
  });

  const closeThreadHistoryOverlay = useLastCallback(() => {
    setIsThreadHistoryOverlayOpen(false);
  });
  ...
  const close = useLastCallback((shouldScrollUp = true) => {
    switch (contentKey) {
      ...
      case RightColumnContent.EditTopic:
        closeEditTopicPanel();
        break;
      case RightColumnContent.AgentChat:
        toggleAgentChat({ force: false });
        break;
      case RightColumnContent.ThreadHistory:
        toggleThreadHistory({ force: false });
        break;
    }
  });
  ...
  function renderContent(isActive: boolean) {
    ...
    switch (renderingContentKey) {
      ...
      case RightColumnContent.AgentChat:
        return <AlphaHumanSmartChatScreen />;
      case RightColumnContent.ThreadHistory:
        return <ThreadHistory onClose={close} isActive={isOpen && isActive} />;
    }
  ...
  return (
    <div
      ...
      className={buildClassName(
        !isChatSelected && 'is-hidden',
        isAgentChat && isThreadHistoryOverlayOpen && 'thread-history-overlay-open',
      )}
    >
      {isOverlaying && (
      ...
      <div id="RightColumn">
        <RightHeader
          ...
          isThreadHistory={isThreadHistory}
          profileState={profileState}
          managementScreen={managementScreen}
          onClose={close}
          onScreenSelect={setManagementScreen}
          onOpenThreadHistoryOverlay={openThreadHistoryOverlay}
        />
        <Transition
          ref={containerRef}
      ...
      {isAgentChat && isThreadHistoryOverlayOpen && (
        <div className="thread-history-overlay">
          <div className="thread-history-overlay-header">
            <Button
              round
              color="translucent"
              size="smaller"
              ariaLabel="Back"
              onClick={closeThreadHistoryOverlay}
              iconName="arrow-left"
            />
            <h3 className="thread-history-overlay-title">Thread History</h3>
          </div>
          <div className="thread-history-overlay-content">
            <ThreadHistory
              isActive={isThreadHistoryOverlayOpen}
              onClose={closeThreadHistoryOverlay}
            />
          </div>
        </div>
      )}
