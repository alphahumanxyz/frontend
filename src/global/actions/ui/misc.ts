...
addActionHandler('toggleAgentChat', (global, actions, payload): ActionReturnType => {
  const { force, tabId = getCurrentTabId() } = payload || {};
  const agentChat = selectTabState(global, tabId).agentChat;
  const willAgentChatBeShown = force !== undefined ? force : !agentChat.isOpen;

  if (willAgentChatBeShown !== agentChat.isOpen) {
    global = updateTabState(global, {
      agentChat: {
        ...agentChat,
        isOpen: willAgentChatBeShown,
      },
    }, tabId);
  }
...
addActionHandler('toggleThreadHistory', (global, actions, payload): ActionReturnType => {
  const { force, tabId = getCurrentTabId() } = payload || {};
  const threadHistory = selectTabState(global, tabId).threadHistory;
  const willThreadHistoryBeShown = force !== undefined ? force : !threadHistory.isOpen;

  if (willThreadHistoryBeShown !== threadHistory.isOpen) {
    global = updateTabState(global, {
      threadHistory: {
        ...threadHistory,
        isOpen: willThreadHistoryBeShown,
      },
    }, tabId);
  }
