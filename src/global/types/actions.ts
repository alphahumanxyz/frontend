...
import type { AgentUpdate, AlphaHumanManagerState, AlphahumanThreadMessage } from '../../lib/alphahuman/types';
...
export interface ActionPayloads {
  ...
  // Summarize
  openSummarizePage: WithTabId | undefined;
  closeSummarizePage: WithTabId | undefined;
  openAlphaHumanThread: {
    threadId: ThreadId;
  } & WithTabId;
  closeAlphaHumanThread: WithTabId | undefined;
  fetchBusiestChats: ({
  ...
  generateSummaries: WithTabId | undefined;
  ...
  resetSummarizeSetup: WithTabId | undefined;
  setSummarizeSelectedDate: {
    date: number | undefined;
  } & WithTabId;

  toggleChatInfo: ({ force?: boolean } & WithTabId) | undefined;
  toggleAgentChat: ({ force?: boolean } & WithTabId) | undefined;
  toggleThreadHistory: ({ force?: boolean } & WithTabId) | undefined;
  setIsUiReady: {
    uiReadyState: 0 | 1 | 2;
  } & WithTabId;
  ...
  // AI Socket
  updateAlphaHumanSocketState: { state: AlphaHumanManagerState };
  onAlphaHumanBackendError: { error: { requestId?: string; message: string; status: number } };
  onAlphaHumanAgentStreamingUpdate: { update: AgentUpdate };
  initAlphaHumanSocket: undefined;
  connectAlphaHumanSocket: { jwtToken: string };
  disconnectAlphaHumanSocket: undefined;
  streamAlphaHumanMessage: { threadId?: string; content: string; chatId?: string };
  startAlphaHumanLogin: undefined;
  stopAlphaHumanLogin: undefined;
  loadAppConfig: {
    hash: number;
  } | undefined;
  ...
  setAgentColumnState: { state: 'expanded' | 'minimized' | 'collapsed' };
  setAgentColumnWidth: { width: number };
  toggleAgentColumn: undefined;
  setContentMode: { mode: 'chat' | 'split' | 'info' };
  setLayoutPinned: { isPinned: boolean };
  loadLayoutPreferences: undefined;
  openAlphaHumanSettings: undefined;
  openUsageStatistics: undefined;
  openOwnProfile: undefined;
  toggleThreadsSidebar: undefined;
  initializeThreadsData: undefined;
  selectThread: { threadId: string };
  selectAlphahumanThreadById: { threadId: string };
  addMessageToAlphahumanThread: { threadId: string; message: AlphahumanThreadMessage };
  openAgentSidebar: { initialMessage?: string };

  // AlphaHuman API actions
  loadAlphahumanMe: undefined;
  loadAlphahumanThreads: undefined;
  loadAlphahumanThreadMessages: { threadId: string };
  loadAlphahumanHighlights: { timeframe: 'daily' | 'weekly' };
  createAlphahumanThread: undefined;

  // Notion OAuth
  connectToNotion: undefined;
  resetNotionOAuthError: undefined;
  completeNotionOAuthSuccess: { workspace: string };
  completeNotionOAuthError: { error: string };

  // Google OAuth
  connectToGoogle: undefined;
  resetGoogleOAuthError: undefined;
  completeGoogleOAuthSuccess: undefined;
  completeGoogleOAuthError: { error: string };
}
