...
import type { AlphaHumanManagerState, AlphahumanThreadMessage } from '../../lib/alphahuman/types';
import type {
  AccountSettings,
...
export type GlobalState = {
  ...
  layout: {
    agentColumnState: 'expanded' | 'minimized' | 'collapsed';
    agentColumnWidth: number;
    contentMode: 'chat' | 'split' | 'info';
    isPinned: boolean;
  };
  ...
  alphahuman: {
    ...
    lastAgentUpdate?: {
      eventType: 'chunk' | 'complete' | 'error' | 'toolStart' | 'toolEnd' | 'reasoning';
      threadId?: string;
      content?: string;
      messageId?: string;
      success?: boolean;
    ...
    threadMessages?: Record<string, { messages: AlphahumanThreadMessage[]; count: number }>;
