...
const WatchFilePlugin = require('@mytonwallet/webpack-watch-file-plugin');
...
const DEFAULT_APP_TITLE = `AlphaHuman${APP_ENV !== 'production' ? ' Alpha' : ''}`;
...
const {
  ...
  BACKEND_API_URL = 'https://staging-api.alphahuman.xyz',
} = process.env;

// Extract backend URL and convert to WebSocket URLs
const getBackendWebSocketUrls = (backendUrl: string): string => {
  try {
...
const backendWebSocketUrls = getBackendWebSocketUrls(BACKEND_API_URL);

const connectSrcIpc = APP_ENV === 'development' ? 'ipc:' : '';
const CSP = `
  default-src 'self';
  connect-src 'self' wss://*.web.telegram.org blob: http: https: wss: ws: ${backendWebSocketUrls} ${connectSrcIpc};
  ...
  return {
    ...
    plugins: [
      ...
      new EnvironmentPlugin({
        ...
        ALPHAHUMAN_BOT_USERNAME: undefined,
        ALPHAHUMAN_BOT_ID: undefined,
      }),
